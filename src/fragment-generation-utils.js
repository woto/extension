...
/**
  ...
  */
const isUniquelyIdentifying = (fragment) => fragments.processTextFragmentDirective(fragment).length === 1;
...
const findWordStartBoundInTextNode = (node, startOffset) => {
  ...
  if (offset < node.data.length
       && fragments.internal.BOUNDARY_CHARS.test(node.data[offset])) return offset;
...
const makeWalkerForNode = (node, endNode) => {
  ...
  const walker = document.createTreeWalker(blockAncestor, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, (node) => fragments.internal.acceptNodeIfVisibleInRange(node));
...
const getTextNodesInSameBlock = (node) => {
  ...
  while (backNode != null && !isBlock(backNode)) {
    ...
    backNode = fragments.internal.backwardTraverse(backWalker, finishedSubtrees);
  }
  preNodes.reverse();
  ...
  if (node.nodeType === Node.TEXT_NODE) {
    ...
    const walker = document.createTreeWalker(node, NodeFilter.SHOW_ELEMENT | NodeFilter.SHOW_TEXT, (node) => fragments.internal.acceptNodeIfVisibleInRange(node));
    walker.currentNode = node;
