  ...
  return (
    <button
      ...
      className={`
        ...
        ${props.state.disabled ? '' : 'hover:border-r-indigo-400 hover:fill-slate-700'}
        ${props.currentTab === props.iconName ? 'opacity-95' : ''}
        ${props.currentTab === props.iconName ? 'bg-gradient-to-br from-white/0 to-slate-600/20' : ''}
        ${props.currentTab === props.iconName ? 'border-r-slate-800/50' : 'border-transparent'}
        group relative border-r-[3px] inline-flex items-center pl-3 py-3
      `}
    >
      <div className="h-5 w-5 relative" dangerouslySetInnerHTML={{ __html: icon }} />
