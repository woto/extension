...
function ResultItem(props: {item: any}) {
  return (
    <div>
      ...
      <div className="text-sm mb-1">
        <a href={props.item.FirstURL}>
          {decodeURI(props.item.FirstURL)}
        </a>
      </div>
    </div>
  );
...
export default function DuckDuckGo(props: {
  ...
  } = useQuery(`DuckDuckGo:${props.q}:${globalContext.apiKey}`, () => {
    ...
    <div className="py-3 space-y-7 break-all">
      <div>
        ...
        {data?.DefinitionURL
          && (
          <div className="text-sm mb-1">
            <a href={data.DefinitionURL}>
              {decodeURI(data.DefinitionURL)}
            </a>
          </div>
          )}
      ...
      {data?.Results.map((item: any, index: number) => <ResultItem key={index} item={item} />)}

      {data?.RelatedTopics.map((item: any, index: number) => (
        <Fragment key={index}>
          { item.Name
            ? item.Topics.map((subitem: any, index: number) => <ResultItem  key={index} item={subitem} />)
