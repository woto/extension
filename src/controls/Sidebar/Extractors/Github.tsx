...
export default function Github(props: {
  ...
  } = useQuery(`Github:${props.q}:${globalContext.apiKey}`, () => {
  ...
  return (
    <div className="py-3 space-y-7 break-all">
      ...
      { data?.repos && data.repos.items.map((item: any) => (
        <div key={item.id}>
          ...
          <div className="text-sm mb-1 space-x-2">
            <div className="inline-flex items-center">
              <div className="w-4 h-4 mr-1 overflow-hidden" dangerouslySetInnerHTML={{ __html: StarIcon }} />
              {formatter.format(item.stargazers_count)}
            </div>

            <div className="inline-flex items-center">
              <div className="w-4 h-4 mr-0.5 overflow-hidden" dangerouslySetInnerHTML={{ __html: ForkIcon }} />
              {formatter.format(item.forks_count)}
            </div>

            <div className="inline-flex items-center">
              <div className="w-4 h-4 mr-1 overflow-hidden" dangerouslySetInnerHTML={{ __html: WatchIcon }} />
