import React, { MouseEventHandler, ReactChildren, ReactNode, SyntheticEvent, useContext, useEffect, useRef, useState } from 'react';
import ReactDOM from "react-dom";
import { GlobalContext } from '../../../Utils';

function Portal(props: { children: ReactNode }) {
  let el = document.querySelector('add-popup-component')!.shadowRoot!.querySelector('#screenshot-portal');
  if (el) {
...
export default function Screenshot(props: {
  ...
  const [rectangle, setRectangle] = useState<{ top: number, right: number, bottom: number, left: number, width: number, height: number }>();
  ...
  useEffect(() => {
    if (!adjustingArea && initialPosition && currentPosition) {
      setRectangle({
        top: currentPosition.top > initialPosition.top ? initialPosition.top : currentPosition.top,
        right: currentPosition.left > initialPosition.left ? window.visualViewport!.width - currentPosition.left : window.visualViewport!.width - initialPosition.left,
        bottom: currentPosition.top > initialPosition.top ? window.visualViewport!.height - currentPosition.top : window.visualViewport!.height - initialPosition.top,
        left: currentPosition.left > initialPosition.left ? initialPosition.left : currentPosition.left,
  ...
  const makeScreenshot = (rectangle: any) => {
    chrome.runtime.sendMessage({
      ...
      coordinates: { x: rectangle.left, y: rectangle.top, width: rectangle.width, height: rectangle.height }
    },
      function ({ imageUri }: { imageUri: string }) {
  ...
  return (
    <>
      ...
      {image &&
        <img className="mt-3 border rounded" src={image} />
      }
      ...
      {choosingArea &&
        <Portal>
          {showCrosshair &&
            <>
              <div className="fixed w-full h-[3px] bg-white/10 backdrop-invert drag-none select-none"
                style={{ top: currentPosition && currentPosition.top - 1 || -999, left: 0 }}></div>
              <div className="fixed h-full w-[3px] bg-white/10 backdrop-invert drag-none select-none"
                style={{ top: 0, left: currentPosition && currentPosition.left - 1 || -999 }}></div>
            </>
          }
          ...
          >
            {rectangle && rectangle.top && rectangle.right && rectangle.bottom && rectangle.left &&
              <>
                <div
                  className="fixed border-2 -m-3 rounded-lg border-slate-50 border-dashed z-20" style={{
                    top: rectangle.top,
                  ...
                  {adjustingArea &&
                    <div className={`absolute translate-x-full drag-none select-none
                      ${rectangle && currentPosition && currentPosition.top > rectangle.top + rectangle.height / 2 ? '-mb-1 bottom-0 translate-y-full' : '-mt-1 top-0 -translate-y-full'}
                      ${rectangle && currentPosition && currentPosition.left > rectangle.left + rectangle.width / 2 ? '-mr-1 right-0' : '-ml-1 -translate-x-full'}
                    `}>
                      <button
                ...
                <div className="bg-black/50 fixed drag-none select-none"
                  style={{ top: 0, right: rectangle.right, bottom: rectangle.bottom + rectangle.height, left: 0 }}
                  ref={topRef}></div>
                <div className="bg-black/50 fixed drag-none select-none"
                  style={{ top: 0, right: 0, bottom: rectangle.bottom, left: rectangle.left + rectangle.width }}
                  ref={rightRef}>
                </div>
                <div className="bg-black/50 fixed drag-none select-none"
                  style={{ top: rectangle.top + rectangle.height, right: 0, bottom: 0, left: rectangle.left }}
                  ref={bottomRef}>
                </div>
                <div className="bg-black/50 fixed drag-none select-none"
                  style={{ top: rectangle.top, right: rectangle.right + rectangle.width, bottom: 0, left: 0 }}
                  ref={leftRef}></div>
              </>
            }
          </div>
