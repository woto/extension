...
import { ArrowCircleLeftIcon, CogIcon, XIcon } from '@heroicons/react/solid';
...
export default function Form(props: {
  ...
  const {
    dispatch: _dispatch,
    operation,
    entity: { entity_id: _entity_id, lookups: _lookups, kinds: _kinds },
  } = props;
  ...
  const _images = useMemo(() => props.entity.images, [props.entity.images]);
  ...
  const [optionalComponents, setOptionalComponents] = useState<
    OptionalComponentsItem[]
  >([]);
  const { add } = useToasts();

  const relevanceOptions: Relevance[] = [
    { id: '0', title: 'Основной объект' },
    { id: '1', title: 'Второстепенный объект' },
    { id: '2', title: 'Один из равнозначных' },
    { id: '3', title: 'Ссылающееся издание или автор' },
  ];
  ...
  const {
    status: MentionDateStatus,
    data: mentionDateData,
    error: MentionDateError,
    isFetching: MentionDateIsFetching,
  ...
  function isOptionalComponentVisible(key: string) {
    return !!optionalComponents.find(
      (object) => object.key === key && object.show === true,
    );
  }
        ...
        <div className="flex">
          ...
          <div className="flex ml-auto flex-grow justify-end">
            ...
            <button
              ...
              disabled={operation === 'edit'}
              onClick={(e) => {
                toggleVisibility(e, 'mentionDate');
              }}
